...
const API = process.env.MEMPALACE_URL || "http://localhost:3111";
...
async function post(path: string, body: Record<string, unknown>, timeoutMs = 5000): Promise<void> {
  try {
    await fetch(`${API}/mempalace${path}`, {
      method: "POST",
      headers: authHeaders(),
    ...
    if (DEBUG) console.error(`[mempalace] POST ${path} failed:`, (e as Error).message);
  }
}

async function postJson(path: string, body: Record<string, unknown>): Promise<unknown | null> {
  try {
    const res = await fetch(`${API}/mempalace${path}`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify(body),
...
All memory tools start with \`mempalace_memory_\`. Use the exact names as they appear in your tool list. Tool results are JSON. Always check what was returned before presenting to the user.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (type === "session.deleted") {
        ...
        if (!sid) {
          if (DEBUG) console.error("[mempalace] session.deleted with no session ID");
          return;
        }

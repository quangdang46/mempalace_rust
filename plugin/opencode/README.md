...
<p align="center">
  ...
  <sub>Persistent cross-session memory via <a href="https://github.com/quangdang46/mempalace">mempalace</a> — 95.2% retrieval accuracy on <a href="https://arxiv.org/abs/2410.10813">LongMemEval-S</a>.</sub>
</p>
...
```json
{
  "plugin": ["./plugins/mempalace-capture.ts"]
}
```
...
cp plugin/opencode/mempalace-capture.ts ~/.config/opencode/plugins/
```
...
1. **Memory context** (once per session): calls `/mempalace/context` and injects project profile, recent session summaries, and important past observations into the system prompt. This is the OpenCode equivalent of Claude's MEMORY.md bridge — instead of syncing to a markdown file, context is injected directly into the system prompt.

2. **File enrichment** (every turn with stashed files): calls `/mempalace/enrich` with files stashed by `tool.execute.before`, `file.edited`, and `message.part.updated` (file parts). File-specific context (past observations, related bugs, semantic search) is injected into the system prompt.
...
| What gets injected | `<mempalace-file-context>` + bug memories | Identical `/enrich` response |
...
```
mempalace  ──write──▶  MEMORY.md  ──read──▶  Claude system prompt
```

- The `claude-bridge/sync` endpoint serializes mempalace observations into a `MEMORY.md` file in the project root
...
```
mempalace  ──push──▶  OpenCode system prompt
```
...
- `AGENTS.md` is a static instruction file for project conventions, coding standards, and tool guidance — mempalace does not read or write it
...
| Team sharing | File is git-trackable, CI-friendly | Memory shared via mempalace server API |
| Integration | Any tool can read MEMORY.md | Requires OpenCode plugin SDK |

### Why OpenCode goes direct

mempalace already persists everything in SQLite (`data/state_store.db`). Adding an intermediate MEMORY.md file would duplicate data, introduce sync lag, and require the model to re-parse structured context from markdown. Direct injection delivers the same data with lower latency and zero staleness — the agent always sees what mempalace knows right now.

...
If neither MCP tool is available, fall back to HTTP: `GET $MEMPALACE_URL/mempalace/sessions` and `POST $MEMPALACE_URL/mempalace/recall` with `Authorization: Bearer $MEMPALACE_SECRET` when set.

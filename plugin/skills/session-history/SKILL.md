...
Fetch recent session history using the `memory_sessions` MCP tool (provided by the mempalace server that this plugin wires up automatically via `.mcp.json`). Pass `limit: 20` to get a meaningful window.

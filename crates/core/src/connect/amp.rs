...
//!
//! Amp stores MCP servers in `~/.config/amp/mcp.json`.
...
impl ConnectAdapter for AmpAdapter {
    ...
    fn config_path(&self) -> PathBuf {
        dirs::home_dir()
            .map(|p| p.join(".config/amp/mcp.json"))
            .unwrap_or_else(|| PathBuf::from("~/.config/amp/mcp.json"))
    }

    fn detect(&self) -> bool {
        dirs::home_dir()
            .map(|p| p.join(".config/amp").exists())
            .unwrap_or(false)
    }

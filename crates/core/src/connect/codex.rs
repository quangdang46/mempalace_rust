...
//!
//! Codex stores MCP servers in `~/.codex/mcp.json`.
...
impl ConnectAdapter for CodexAdapter {
    ...
    fn config_path(&self) -> PathBuf {
        dirs::home_dir()
            .map(|p| p.join(".codex/mcp.json"))
            .unwrap_or_else(|| PathBuf::from("~/.codex/mcp.json"))
    }

    fn detect(&self) -> bool {
        dirs::home_dir()
            .map(|p| p.join(".codex").exists())
            .unwrap_or(false)
    }

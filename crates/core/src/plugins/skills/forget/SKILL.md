---
name: forget
description: Delete specific observations or sessions from mempalace. Use when user says "forget this", "delete memory", or wants to remove specific data for privacy.
...
1. First search for matching observations with the `memory_smart_search` MCP tool. Use the user's input as the `query` with `limit: 20`.
...
3. Once confirmed, call `memory_governance_delete` with:
   - `memoryIds: [<id>, ...]` — an array of the memory IDs returned by the search in step 1
...
**Never delete without explicit user confirmation.** If the MCP tools aren't available, the mempalace MCP shim didn't start — tell the user to:
1. Run `mpr plugin list` and confirm `mempalace` shows as enabled.
2. Restart the agent (the plugin's `.mcp.json` is only read on startup).

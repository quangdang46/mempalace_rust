...
Fetch recent session history using the `memory_sessions` MCP tool. Pass `limit: 20` to get a meaningful window.
...
Format as a clean timeline. **Do NOT make up sessions** — only show what the MCP tool actually returned. If `memory_sessions` isn't available, the mempalace MCP shim didn't start — tell the user to:
